...
package: tidy
	...
	cd ./build/PAYLOAD && zip -r NextUI.zip .system .tmp_update Tools
	mv ./build/PAYLOAD/NextUI.zip ./build/BASE
	
	# TODO: can I just add everything in BASE to zip?
	# cd ./build/BASE && zip -r ../../releases/$(RELEASE_NAME)-base.zip Bios Roms Saves miyoo miyoo354 trimui rg35xx rg35xxplus gkdpixel miyoo355 magicx em_ui.sh NextUI.zip README.txt
	cd ./build/BASE && zip -r ../../releases/$(RELEASE_NAME)-base.zip Bios Roms Saves trimui em_ui.sh NextUI.zip README.txt
